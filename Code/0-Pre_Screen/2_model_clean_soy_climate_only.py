# -*- coding: utf-8 -*-
"""2_Model_Clean_Soy_ClimateOnly_20seeds.py

Runs only Soybean's climate-only model (no time feature and no CO2 feature),
used later to compute the residual of soybean yield minus climate-predicted yield,
then do growth-rate correlation against corn's same residual to pick reliable counties.

- 20 random seeds (0–19)
- No corn trend merge / reliable county filtering —— trained on all of soy's raw data
...
results_folder_name = "Results_Soy_ClimateOnly_20seeds"
...
def nn_model(df, input, output):
    ...
    model.add(Dense(1))
    model.compile(
        optimizer=Adam(learning_rate=3e-4),  # Soy uses 3e-4 (consistent with the original script)
